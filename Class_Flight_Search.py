...
class FlightSearch:
    ...
    def check_flights(self,origin_city_code,destination_city_code, from_time, to_time):
        ...
        query = {
            ...
            "departureDate": from_time.strftime("%Y-%m-%dT%H:%M:%SZ"),
            "returnDate": to_time.strftime("%Y-%m-%dT%H:%M:%SZ"),
            "adults": 1,
            "nonStop": "true",
            "currencyCode": "USD",
            "max":"10",
        }#SO basically if no errors send the json response otherwise send nothing but the error.
        response = requests.get(url=IATA_ENDPOINT, headers=headers, params=query)
        if response.status_code != 200: #ie if not okay,
            print(f"check_flights() response code: {response.status_code}")
            print("There was a problem with the flight searching!\n""For details on status codes, check the API documentation:\n"
